...
def get_x_coordinate(width, year_index):
    ...
    """
    middle_width = (width-GRAPH_MARGIN_SIZE*2) // len(YEARS)
    x_coordinate = GRAPH_MARGIN_SIZE + middle_width * year_index
    return x_coordinate
    ...
    for name in lookup_names:
        ...
        for i in range(len(YEARS)):
            ...
            if str(YEARS[i]) not in name_data[name]:
                ...
                y_coordinate = GRAPH_MARGIN_SIZE + int(name_data[name][str(YEARS[i])]) // 2
                text = f'{name} {name_data[name][str(YEARS[i])]}'

            canvas.create_text(x_coordinate+TEXT_DX, y_coordinate, text=text, anchor=tkinter.NW, fill=color)
            if i > 0:
                canvas.create_line(old_x, old_y, x_coordinate, y_coordinate, width=LINE_WIDTH, fill=color)
            old_x, old_y = x_coordinate, y_coordinate
